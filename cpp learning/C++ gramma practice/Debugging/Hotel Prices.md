...
上一行代码一直调用的是基类Hotel Room的get_price函数，Hotel Apartment的get_price函数一直未被调用，导致apartment类型的房间费用计算错误。为了Hotel Apartment的get_price函数被调用，给Hotel Room的get_price函数增加virtual关键字。